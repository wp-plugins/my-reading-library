...
function query_amazon( $query ) {
    ...
    require_once MRL_INCLUDES_DIR . 'secure.php';
   ...
   $request = "http://" . $host . $uri . "?" . $request . "&Signature=" . $signature;
    ...
    if ( $options['httpLib'] == 'curl' ) {
        if ( !function_exists('curl_init') ) {
            return new WP_Error('curl-not-installed', __('cURL is not installed correctly.', MRLTD));
        } else {
            $ch = curl_init();

            curl_setopt($ch, CURLOPT_URL, $request);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_USERAGENT, 'My Reading Library' . MY_READING_LIBRARY_VERSION);
        ...
        $snoopy = new snoopy;
        $snoopy->agent = 'My Reading Library' . MY_READING_LIBRARY_VERSION;
    ...
    if ( empty($xmlString) ) {
        do_action('mrl_search_error', $query);
        echo '
        <div id="message" class="error fade">
            <p><strong>' . __("Oops!", MRLTD) . '</strong></p>
            <p>' . sprintf(__("For some reason, I couldn't search for your book on amazon%s.", MRLTD), $options['domain']) . '</p>
            <p>' . __("Amazon's Web Services may be down, or there may be a problem with your server configuration.", MRLTD) . '</p>

                    ';
        if ( $options['httpLib'] )
            echo '<p>' . __("Try changing your HTTP Library setting to <strong>cURL</strong>.", MRLTD) . '</p>';
        echo '
        </div>
        ';
        return false;
    ...
    require_once MRL_XML_DIR . 'IsterXmlSimpleXMLImpl.php';
    ...
    foreach ($items as $item)
    {
        ...
        foreach ($editions as $edition)
        {
            ...
            if (empty($image))
            {
                $image = get_option('siteurl') . '/wp-content/plugins/my-reading-library/no-image.png';
            }

// Added Begin
            $size = "{$options['limageSize']}Image";
            if (empty($item->$size))
            {
                continue;
            }

            $limage = $item->$size->URL->CDATA();
            if (empty($limage))
            {
                $limage = get_option('siteurl') . '/wp-content/plugins/my-reading-library/no-image.png';
            }
// Added End
            ...
            $results[] = apply_filters('raw_amazon_results', compact('author', 'title', 'binding', 'ed', 'date', 'publisher', 'image', 'limage', 'asin')); // Added 'limage'
        }
    }
