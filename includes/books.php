...
function get_books($query, $show_private = false) {
    ...
    foreach ( (array) $books as $book ) {
        $book->added = ( mrl_empty_date($book->added) )	? '' : $book->added;
        $book->started = ( mrl_empty_date($book->started) )	? '' : $book->started;
        $book->finished = ( mrl_empty_date($book->finished) )	? '' : $book->finished;
    }
...
function update_book( $query ) {
    ...
    if ( $insert ) {
        ...
        $wpdb->query("
		INSERT INTO {$wpdb->prefix}my_reading_library
		($columns)
		VALUES($values)
            ");

        $id = $wpdb->get_var("SELECT MAX(b_id) FROM {$wpdb->prefix}my_reading_library");
