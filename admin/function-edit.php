...
switch ( $action ) {
    case 'delete':
        ...
        check_admin_referer('my-reading-library-delete-book_' . $id);

        $wpdb->query("
		DELETE FROM {$wpdb->prefix}my_reading_library
		WHERE b_id = $id
            ");

        wp_redirect($mrl_url->urls['manage'] . '&deleted=1');
        die;
        break;

    case 'update':
        check_admin_referer('my-reading-library-edit');
...
// Added End
            ...
            $added			= ( mrl_empty_date($_POST['added'][$i]) )	? '' : $wpdb->escape(date('Y-m-d H:i:s', strtotime($_POST['added'][$i])));
            $started		= ( mrl_empty_date($_POST['started'][$i]) )	? '' : $wpdb->escape(date('Y-m-d H:i:s', strtotime($_POST['started'][$i])));
            $finished		= ( mrl_empty_date($_POST['finished'][$i]) )	? '' : $wpdb->escape(date('Y-m-d H:i:s', strtotime($_POST['finished'][$i])));
...
// Added End
			...
			SELECT b_status
			FROM {$wpdb->prefix}my_reading_library
			WHERE b_id = $id
                ");
			...
			$query = "
			UPDATE {$wpdb->prefix}my_reading_library
			SET
                $started
                $finished
                $rating
...
// Added End
        ...
        if ( empty($referer) )
            $forward = $mrl_url->urls['manage'] . '&updated=' . $updated;
        else
