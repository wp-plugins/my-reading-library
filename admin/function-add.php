...
if ( !empty($_POST['amazon_data']) ) {
    ...
    check_admin_referer('my-reading-library-add');
    ...
    check_admin_referer('my-reading-library-manual-add');
    ...
    if ( !empty($_POST['custom_image']) ) {
        ...
        $b_image = get_option('siteurl') . '/wp-content/plugins/my-reading-library/no-image.png';
        $b_limage = get_option('siteurl') . '/wp-content/plugins/my-reading-library/no-image.png';
	}
    $b_asin = '';
    $b_added = date('Y-m-d H:i:s');
    $b_status = 'unread';
    ...
    $id = add_book($query);
    if ( $id > 0 ) {
        wp_redirect($mrl_url->urls['add'] . '&added=' . intval($id));
        die;
    } else {
        wp_redirect($mrl_url->urls['add'] . '&error=true');
        die;
    }
}
